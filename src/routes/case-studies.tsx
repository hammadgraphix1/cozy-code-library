    ...
    <div className="bg-[#0A0A0A] text-white min-h-screen">
      ...
      <section className="pt-40 pb-20">
        <div className="mx-auto max-w-[1600px] px-5 sm:px-10">
          <span className="text-[10px] uppercase tracking-[0.3em] text-[#C8FF00]">Case Studies</span>
        ...
        <div className="mx-auto max-w-[1600px] px-5 sm:px-10 space-y-6">
          {cases.map((p, i) => (
            <Link key={p.slug} to="/work/$slug" params={{ slug: p.slug }} className="group block">
              <div className="grid gap-8 lg:grid-cols-[1fr_1.2fr] rounded-3xl border border-white/8 bg-[#111] p-6 sm:p-10 hover:border-[#C8FF00]/30 transition-colors">
                ...
                <div className="flex flex-col justify-between">
                  <div>
                    <div className="flex items-center gap-3 text-[10px] uppercase tracking-[0.25em] text-white/50">
                      <span className="text-[#C8FF00]">Case 0{i + 1}</span>
                      <span>·</span>
                      <span>{p.category}</span>
                    </div>
                    <h2 className="mt-5 font-display text-4xl sm:text-6xl leading-[0.95] group-hover:text-[#C8FF00] transition-colors">{p.title}</h2>
                    <p className="mt-5 text-white/65 leading-relaxed">{p.caseStudy!.overview}</p>
                  </div>
                  <div className="mt-8 rounded-2xl bg-[#C8FF00] text-black p-5">
