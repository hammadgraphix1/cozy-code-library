    ...
    <div className="bg-[#0A0A0A] text-white min-h-screen">
      ...
      <section className="pt-40 pb-16">
        <div className="mx-auto max-w-[1600px] px-5 sm:px-10">
          <span className="text-[10px] uppercase tracking-[0.3em] text-[#DFFF00]">Contact</span>
          <h1 className="mt-4 font-display text-[14vw] sm:text-[10vw] leading-[0.88] tracking-[-0.04em]">
            Let's make<br/>something<br/><em className="not-italic text-[#DFFF00]">memorable.</em>
          </h1>
        </div>
      </section>
          ...
          <div className="space-y-3">
            {channels.map((c) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="group flex items-center justify-between gap-6 rounded-3xl border border-white/8 bg-[#111] p-6 sm:p-10 hover:border-[#DFFF00]/40 hover:bg-[#141414] transition-colors"
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] uppercase tracking-[0.25em] text-white/45">{c.label}</span>
                    {c.badge && <span className="rounded-full bg-[#DFFF00] text-black text-[9px] font-bold uppercase tracking-[0.2em] px-2 py-0.5">{c.badge}</span>}
                  </div>
                  <p className="mt-3 font-display text-3xl sm:text-5xl leading-tight truncate group-hover:text-[#DFFF00] transition-colors">{c.value}</p>
                </div>
                <span className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-white/15 group-hover:bg-[#DFFF00] group-hover:border-[#DFFF00] group-hover:text-black transition-all">↗</span>
              </a>
            ))}
          </div>
          ...
          <div className="space-y-6">
            <div className="rounded-3xl border border-white/8 bg-[#111] p-8">
              <div className="flex items-center gap-4">
                <span className="relative h-14 w-14 overflow-hidden rounded-full ring-2 ring-[#DFFF00]">
              ...
              <div className="mt-6 flex flex-wrap gap-2 text-[10px] uppercase tracking-[0.2em]">
                ...
                <span className="rounded-full border border-[#DFFF00]/30 px-3 py-1.5 text-[#DFFF00]">Booking Now</span>
              </div>
            </div>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="block rounded-3xl bg-[#DFFF00] text-black p-8 glow-yellow hover:scale-[1.02] transition-transform"
            >
