        ...
        <div className="space-y-16">
          {grouped.map(({ cat, items }) => (
            <div key={cat}>
              ...
              <div className={cn("grid gap-4", cat === "Thumbnail Design" ? "grid-cols-2 sm:grid-cols-2 lg:grid-cols-2" : "grid-cols-2 sm:grid-cols-3 lg:grid-cols-4")}>
                {items.map(p => (
                  <button key={p.slug} onClick={() => setLightbox(p.image)} className="group relative overflow-hidden rounded-xl bg-[#111] border border-white/5 hover:border-[#C8FF00]/40 transition-all duration-300">
                    <div className={cn("relative overflow-hidden", cat === "Thumbnail Design" ? "aspect-video" : "aspect-square")}>
